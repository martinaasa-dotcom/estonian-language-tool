...
- **21 ways to practice, over one deck.** Six rounds against the clock (a 60-second Case
  ...
  table, the cards you keep failing, and the words you looked up yourself rather than met in a
  unit, which the queue is otherwise slowest to reach. Everything writes to the same review log,
  so a game still moves the schedule forward.
