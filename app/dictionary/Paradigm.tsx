...
function VerbTable({ forms }: { forms: ParadigmForm[] }) {
  ...
  return (
    <>
      ...
      {nonFinite.length > 0 && (
        <div className="mt-3 grid gap-px overflow-hidden rounded-md border" style={{ borderColor: "var(--rule)", background: "var(--rule)", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))" }}>
          {nonFinite.map(({ form, slot }) => (
            <div key={form.morphCode} className="px-3 py-2.5" style={{ background: "var(--surface)" }}>
