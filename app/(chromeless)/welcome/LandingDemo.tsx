...
const GRADES = [
  { label: "Again", key: "1", tone: "var(--peach)", soft: "var(--peach-soft)", next: "in 1 min" },
  { label: "Hard", key: "2", tone: "var(--butter)", soft: "var(--butter-soft)", next: "in 4 days" },
  { label: "Good", key: "3", tone: "var(--mint)", soft: "var(--mint-soft)", next: "in 12 days" },
  { label: "Easy", key: "4", tone: "var(--sky)", soft: "var(--sky-soft)", next: "in 28 days" },
] as const;
...
export function DemoCard({ words }: { words: DemoWord[] }) {
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <div key={`${i}-${revealed}`} className="pop-in flex min-h-[232px] flex-col items-center justify-center gap-3 px-6 py-9 text-center">
          <div className="flex items-center gap-2">
            <p lang="et" className="est text-[42px] font-bold leading-none tracking-tight" style={{ color: "var(--ink)" }}>
              {word.lemma}
            </p>
            <span
              aria-hidden
          ...
          <div className="flex flex-wrap justify-center gap-1.5">
            {word.cefr && (
              <span className="label-xs rounded-full px-2.5 py-1" style={{ background: "var(--sky-soft)", color: "var(--sky)" }}>
                {word.cefr}
              </span>
            )}
            {word.gradationNote && (
              <span
                ...
                style={{ background: "var(--butter-soft)", color: "var(--butter)", textTransform: "none" }}
              >
          ...
          <div aria-live="polite" className="min-h-[44px]">
            {revealed && (
              <p className="est pop-in text-[26px] font-bold" style={{ color: "var(--accent)" }}>
                {word.translation}
              </p>
            )}
          </div>
        ...
        <div className="border-t p-3.5" style={{ borderColor: "var(--rule-soft)" }}>
          {!revealed ? (
            <button
              ...
              className="grad-accent press w-full rounded-full py-3 text-[15px] font-semibold transition-all hover:brightness-105"
            ...
            <div className="grid grid-cols-4 gap-2">
              {GRADES.map((g) => (
                ...
                >
                  ...
                  <span className="tnum text-[10.5px] opacity-80">{g.next}</span>
                </button>
              ))}
            </div>
          )}
      ...
      <p className="relative mt-9 flex items-center justify-center gap-4 text-[11.5px]" style={{ color: "var(--ink-3)" }}>
      ...
      <div className="flex flex-wrap items-center gap-2 border-b px-5 py-4" style={{ borderColor: "var(--rule-soft)" }}>
        ...
        {words.map((w, n) => (
          <button
            ...
            lang="et"
            className="est press rounded-full px-3.5 py-1.5 text-[15px] transition-all"
            style={{
              background: active === n ? "var(--accent)" : "var(--raised)",
        ...
        <div>
          ...
          <div className="flex flex-col gap-2">
            {word.principal.map((p) => (
              ...
              >
                <span className="text-[12px]" style={{ color: "var(--ink-3)" }}>{p.label}</span>
                <span lang="et" className="est text-[19px] font-bold" style={{ color: "var(--ink)" }}>{p.value}</span>
              </div>
            ))}
          </div>
          <p
            className="mt-4 flex items-start gap-2 rounded-[var(--r)] px-4 py-3 text-[13px] leading-relaxed"
        ...
        <div>
          ...
          <ul key={word.lemma} className="fade-up grid grid-cols-2 gap-1.5">
            {word.cases.filter((c) => !c.principal && c.singular).map((c) => (
              ...
              >
                <span className="text-[11px]" style={{ color: "var(--accent-deep)", opacity: 0.75 }}>{c.en}</span>
                <span lang="et" className="est text-[15px] font-semibold" style={{ color: "var(--accent-deep)" }}>
                  {c.singular}
                </span>
              </li>
            ))}
...
export function TutorPeek() {
  ...
  return (
    <div className="flex flex-col gap-3">
      <div
        className="ml-auto max-w-[85%] rounded-[var(--r-lg)] rounded-br-md px-4 py-3 text-[14px]"
      ...
      {asked ? (
        <div
          className="fade-up max-w-[92%] rounded-[var(--r-lg)] rounded-bl-md border px-4 py-3 text-[14px] leading-relaxed"
          style={{ background: "var(--surface)", borderColor: "var(--rule)", color: "var(--ink-2)" }}
        >
          <span className="label-xs mb-1.5 block" style={{ color: "var(--blush)" }}>Anu</span>
        ...
        <button
          ...
          className="press mr-auto flex items-center gap-2 rounded-full border px-4 py-2 text-[13.5px] font-semibold transition-all hover:-translate-y-px"
          style={{ background: "var(--surface)", borderColor: "var(--rule)", color: "var(--ink-2)" }}
        >
