...
| Data model | A word has one or more *paradigms*, each containing *forms* with orthographic representations, transcriptions and sound-file links — exactly the shape our principal-parts model needs |
...
- Every response's `usage` is written to `Message` and `UsageDay` — spend is measured, not estimated.
