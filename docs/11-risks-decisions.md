...
| R2 | Ekilex paradigm data does not yield all five noun principal parts | Low–Medium | High | Open | Phase 0 spike answers this in week one; fallback is to store what exists and mark gaps explicitly rather than guess |
...
| R12 | Single-user local DB means no access from another device | High | Low–Medium | **Open — needs a decision** | `12-open-questions.md` Q1; Phase 5 migration path exists |
...
- **Ekilex key refused** → the dictionary becomes user-curated with AI assistance and an explicit
