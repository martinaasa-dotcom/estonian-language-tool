...
export function StatTile({ value, label, tone = "accent", icon, hint }: {
  ...
  return (
    <div
      className="flex flex-col gap-1 rounded-[var(--r)] px-4 py-3.5"
      style={{ background: CARD_TONES[tone].background }}
    >
      <div className="flex items-center justify-between gap-2">
        <span className="label-xs" style={{ color: fg, opacity: 0.85 }}>{label}</span>
        {icon && <span style={{ color: fg, opacity: 0.7 }}>{icon}</span>}
      </div>
