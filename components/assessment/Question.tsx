...
export function EstonianPrompt({ text }: { text: string }) {
  ...
  return (
    <p lang="et" className={`est mt-5 ${size} font-bold leading-snug`} style={{ color: "var(--ink)" }}>
      {parts.map((part, i) => (
        <span key={i}>
          {i > 0 && (
            <span
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-lg" style={{ color: "var(--ink-2)" }}>
        <span lang="et" className="est font-bold" style={{ color: "var(--ink)" }}>{item.lemma}</span>
      ...
      {mark === null ? (
        ...
        <div className="pop-in mt-6">
          ...
          */}
          <p lang="et" className="est mt-4 text-xl font-bold leading-snug" style={{ color: "var(--ink)" }}>
            {item.full}
          </p>
          <Provenance source={item.source} />
          <Button variant="primary" size="lg" className="mt-5" autoFocus onClick={() => onAnswer({ credit: mark.credit })}>
