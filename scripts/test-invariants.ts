...
/*
  ...
  everything that arrives with a width of its own, on twenty-three routes at
  two widths, and asks whether any of them is cut off by an ancestor that
  clips, drawn outside a border somebody painted, or resized away from the
  size it declared. Then it swaps every run of text for a run of letters OF
  THE SAME LENGTH with no space and no hyphen in it and asks all three again,
  ...
  width has to fit as well. With the four declarations removed it failed 93 of
  its 192 checks, which is how anybody knows it is looking.
*/
