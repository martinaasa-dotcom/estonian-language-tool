...
export interface DemoStems {
  ...
  /**
   * The short illative the dictionary records, or `null` where it records
   * none. Required rather than optional, so that a word added here cannot
   * silently skip the question: `NounStems` is the same shape for the same
   * reason. `sõber` has one recorded and it is spelled like its partitive,
   * which `singularForms` explains and handles.
   */
