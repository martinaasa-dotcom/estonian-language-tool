import { CASES, caseOptionLabel, type CaseSpec } from "@/lib/estonian/cases";
...
export function pickOptions<T extends Textual>(input: {
  ...
  */
  ...
  const options = shuffled([answer, ...wrong], rng).map((o) => o.text);
  return { options, answer: options.indexOf(answer.text) };
}

export function shuffled<T>(list: readonly T[], rng: () => number): T[] {
  const out = [...list];
...
/** A case, labelled the way a class names one. */
export interface CaseOption extends Textual {
  readonly spec: CaseSpec;
}

export const CASE_OPTIONS: readonly CaseOption[] = CASES.map((spec) => ({
  text: caseOptionLabel(spec),
  spec,
}));

export function caseOptionFor(spec: CaseSpec): CaseOption {
  return CASE_OPTIONS.find((o) => o.spec.key === spec.key) ?? { text: caseOptionLabel(spec), spec };
}
 ...
 * kaasaütlev, which is the set `ASKABLE` in `items.ts` asks about and roughly
 * what a beginner's course covers. Rajav, olev and ilmaütlev a learner can
 * cross out without looking at the form in front of them, simply because their
 * class has not got there.
 *
 ...
 */
export function caseNearness(candidate: CaseOption, answer: CaseOption): number {
  const a = candidate.spec;
  const b = answer.spec;
  let score = 0;

  const asked = new Set(b.question.split(/\s+/));
  for (const word of a.question.split(/\s+/)) if (asked.has(word)) score += 5;

  if (a.suffix && b.suffix && (a.suffix.startsWith(b.suffix) || b.suffix.startsWith(a.suffix))) score += 3;
  if (a.principal === b.principal) score += 2;
  if (familiar(a) === familiar(b)) score += 2;
  if (a.suffix.length === b.suffix.length) score += 1;
  return score;
}
