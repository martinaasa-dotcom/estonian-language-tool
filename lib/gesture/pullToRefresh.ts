/*
  ...
  It is borrowed wholesale from Upside Lab, numbers included, because the
  numbers are the part that was arrived at rather than chosen, and a gesture
  that feels different in two of the same person's apps is a gesture that
  feels wrong in one of them.
*/
...
/*
  ...
  A refresh answered from cache can come back in 40ms, and a ring that appears
  and vanishes inside two frames reads as a glitch rather than as an answer,
  so the floor buys it enough time to be seen and understood. The ceiling is
  the other failure: a request that never settles would otherwise leave the
  ring turning for the rest of the session, and a spinner that never stops is
  the app lying about still trying.
*/
